 ...
 *
 *         The program sets up two pairs of LEDs (led1, led2) and (led3, led4) 
 *         along with two buttons (button1 and button2) connected to GPIO pins. 
 *         Two threads are created to continuously toggle the LEDs based on button 
 *         presses. When a button is pressed, the corresponding LED pair toggles 
 *         its state (on/off), and the program prints a message indicating whether 
 *         the LEDs are enabled or disabled.
 *
 *         GPIO line configuration and thread management are handled in this 
 *         program to achieve the LED control functionality.
 *
 ...
 *         Initializes GPIO lines for two pairs of LEDs (led1, led2) and 
 *         (led3, led4) and two buttons (button1 and button2).
 *         Defines thread arguments (args1 and args2) for controlling the LEDs and 
 *         buttons. Creates two threads (t1 and t2) to run the `run_thread` 
 *         function concurrently for each LED pair and button pair.
